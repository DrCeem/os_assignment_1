...
int main(int argc, char *argv[]) 
{
    ...
    //Open the shared memory (created by proccess A)
    file_descriptor = shm_open(shared_memory_path, O_RDWR, 0);
    ...
    while (running)
    {
        ...
        for(int package_id = 0; package_id < package_num; package_id++)
        {
            ...
            if(package_id > 0)
            {
                ...
                if(sem_wait(&shared_memory_pointer->semaphore3)==-1) 
                    errExit("sem_wait");
            }
            ...
            //TESTING
            // printf("\nProccess B: Package id =%d, Packege contents\n",package_id);
            ...
            shared_memory_pointer->writen_by_A = false;

            //Post the semaphore to let peer know it can read the contents of the shared memory
            if(sem_post(&shared_memory_pointer->semaphore4) == -1)
                errExit("sem_post");   
        ...
        //TESTING 
        // printf("Shared memory after Proccess B insertion\n");
    ...
    {        
        ...
        if((shared_memory_pointer->writen_by_A) && (shared_memory_pointer->lenght > 0)) {
            ...
            while (1) 
            {
                ...
                if(sem_wait(&shared_memory_pointer->semaphore2) == -1)
                    errExit("sem_wait");
                ...
                if(sem_post(&shared_memory_pointer->semaphore1) == -1)
                    errExit("sem_post"); 
            }
